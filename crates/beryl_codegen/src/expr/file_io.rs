...
pub fn gen_read_file<'ctx>(
    ...
    let file_open_fn = ctx
        .module
        .get_function("beryl_file_open")
        .unwrap_or_else(|| {
            let fn_type = i8_ptr_type.fn_type(&[i8_ptr_type.into(), i64_type.into()], false);
            ctx.module.add_function("beryl_file_open", fn_type, None)
        });

    let file_read_fn = ctx
        .module
        .get_function("beryl_file_read_all")
        .unwrap_or_else(|| {
            ...
            ctx.module
                .add_function("beryl_file_read_all", fn_type, None)
        });

    let file_close_fn = ctx
        .module
        .get_function("beryl_file_close")
        .unwrap_or_else(|| {
            let fn_type = ctx
                .context
                .void_type()
                .fn_type(&[i8_ptr_type.into()], false);
            ctx.module.add_function("beryl_file_close", fn_type, None)
        });

    // 3. 调用 beryl_file_open(path, 0) 读模式
...
pub fn gen_write_file<'ctx>(
    ...
    let file_write_fn = ctx
        .module
        .get_function("beryl_file_write")
        .unwrap_or_else(|| {
            let fn_type = i64_type.fn_type(&[i8_ptr_type.into(), i8_ptr_type.into()], false);
            ctx.module.add_function("beryl_file_write", fn_type, None)
        });
    ...
    // 3. 调用 beryl_file_open(path, 1) 写模式

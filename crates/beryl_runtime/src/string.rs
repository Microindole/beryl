//! Beryl String Runtime
//!
//! 提供 Beryl 语言的字符串处理运行时支持
...
#[no_mangle]
pub unsafe extern "C" fn beryl_string_len(ptr: *const c_char) -> i64 {
    if ptr.is_null() {
        return 0;
    }
...
/// `ptr` must be a valid null-terminated C string
#[no_mangle]
pub unsafe extern "C" fn beryl_string_trim(ptr: *const c_char) -> *mut c_char {
    if ptr.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn beryl_string_split(
    ...
    let vec = Box::into_raw(BerylVec::new(parts.len() as i64));
    ...
    for part in parts {
        ...
        if !part_ptr.is_null() {
            unsafe {
                ...
                *part_ptr.add(len) = 0;
                // 将指针作为 i64 存储 (因为 BerylVec 存储 i64)
...
///
/// # Safety
/// `vec_ptr` must be a valid BerylVec containing string pointers
/// `sep_ptr` must be a valid null-terminated C string
#[no_mangle]
pub unsafe extern "C" fn beryl_string_join(
    vec_ptr: *const BerylVec,
...
/// `ptr` must be a valid null-terminated C string
#[no_mangle]
pub unsafe extern "C" fn beryl_string_substr(
    ptr: *const c_char,
    start: i64,
    len: i64,
) -> *mut c_char {
...
mod tests {
    ...
    fn test_string_len() {
        ...
        assert_eq!(unsafe { beryl_string_len(s.as_ptr()) }, 5);

        let empty = CString::new("").unwrap();
        assert_eq!(unsafe { beryl_string_len(empty.as_ptr()) }, 0);
    }

    #[test]
    fn test_string_trim() {
        let s = CString::new("  hello world  ").unwrap();
        let result = unsafe { beryl_string_trim(s.as_ptr()) };
    ...
    fn test_string_split() {
        ...
        let vec = unsafe { beryl_string_split(s.as_ptr(), delim.as_ptr()) };
        assert!(!vec.is_null());
    ...
    fn test_string_join() {
        // 创建 vec 并填充
        let vec = Box::into_raw(BerylVec::new(3));
        ...
        unsafe {
            ...
            let result = beryl_string_join(vec, sep.as_ptr());
            assert!(!result.is_null());
    ...
    fn test_string_substr() {
        ...
        let result = unsafe { beryl_string_substr(s.as_ptr(), 0, 5) };
        assert!(!result.is_null());
        let substr = unsafe { CStr::from_ptr(result) }.to_str().unwrap();
        assert_eq!(substr, "hello");
        unsafe { libc::free(result as *mut libc::c_void) };

        let result2 = unsafe { beryl_string_substr(s.as_ptr(), 6, 5) };

...
//!
//! 语句代码生成器，将 Lency 语句转换为 LLVM IR

mod control_flow;

use inkwell::values::PointerValue;
use lency_syntax::ast::Expr;
...
pub struct StmtGenerator<'ctx, 'a> {
    ...
    /// 局部变量表
    pub(crate) locals: &'a mut HashMap<String, (PointerValue<'ctx>, lency_syntax::ast::Type)>,
    /// 循环上下文栈
...
impl<'ctx, 'a> StmtGenerator<'ctx, 'a> {
    ...
    pub fn new(
        ...
        locals: &'a mut HashMap<String, (PointerValue<'ctx>, lency_syntax::ast::Type)>,
        return_type: &'a Type,
    ) -> Self {
        Self {
            ctx,
